...
def save_result(path):
    if not os.path.exists(path):
        os.makedirs(path)
    LDA_docs_list = np.array(docs_list, dtype=object)
...
def run_clda(t_data, start, end_iter, iterations, save_p, clip, c_len, palpha, pbeta, pgamma):
    ...
    def dominant_topics_in_each_doc(topic_list, num_topics):
        dom_num = num_topics*[0]
        for item in topic_list:
            dom_num[item[1]] += 1
        max_value = max(dom_num)
        max_index = dom_num.index(max_value)
        if sum(dom_num) > 0:
            perc_contrib = dom_num[max_index]/sum(dom_num)
        else:
            perc_contrib = 0
        return max_index, perc_contrib

    def get_summary(LDA_docs_list):
        dom_top = []
        perc_c = []
        summary_all = []
        for i in range(len(LDA_docs_list)):
            summary = []
            dom_t = dominant_topics_in_each_doc(LDA_docs_list[i], topic_num)
            for item in LDA_docs_list[i]:
                if item[1] == dom_t[0]:
                    summary.append(index_word[item[0]])
            summary_all.append(" ".join(summary))
            dom_top.append(dom_t[0])
            perc_c.append(dom_t[1])
        return dom_top, perc_c, summary_all
    ...
    summary_ = get_summary(LDA_docs_list)

    import pandas as pd

    p_df = pd.DataFrame(list(zip(range(len(summary_[0])), summary_[0], summary_[1], summary_[2])),
               columns =['doc_num', 'dominant_topic', 'perc_contrib', 'sentence'])

    p_df['Text'] = [item.split(" ") for item in p_df['sentence']]

    return summary_[2], p_df
